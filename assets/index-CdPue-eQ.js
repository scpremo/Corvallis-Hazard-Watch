...
`,Ft=()=>(Ce(),e.jsx(kt,{children:e.jsxs("ul",{className:"links",children:[e.jsx("li",{className:"start",children:e.jsx(te,{to:"/Corvallis-Hazard-Watch/",title:"Home",children:"Home"})}),e.jsx("li",{children:e.jsx(te,{to:"/Corvallis-Hazard-Watch/map",title:"Map",children:"Map"})}),e.jsx("li",{children:e.jsx(te,{to:"/Corvallis-Hazard-Watch/dataView",className:"dataView",children:"DataView"})}),e.jsx("li",{children:e.jsx(te,{to:"/Corvallis-Hazard-Watch/about",title:"About",children:"About"})})]})})),Et=({id:r,created_at:t,type:o,latitude:a,longitude:s,text:d,radius:m,location:x,onClick:v})=>{var y;const I=M(De),w=_.td`
  ...
  `;u.useEffect(()=>{const c=[];r.forEach(l=>{c.push(JSON.parse(JSON.stringify(l)))}),[...c].sort((l,g)=>new Date(g.created_at)-new Date(l.created_at)),o(c)},[r]);const X=(c,l,g,p,C)=>{console.log(C),p=z(c,l,p),p=pe(g,p),C&&C.length>0&&(p=he(p,C));const S=[...p].sort((H,J)=>new Date(J.created_at)-new Date(H.created_at));o(S);const T=document.getElementById("locationSearch");T.value=C},he=(c,l)=>{console.log("filter by name: ",l);var g=l.split(",").map(p=>p.trim().toLowerCase());return c=c.filter(p=>{var C=p.location.split(",").map(T=>T.trim().toLowerCase());let S=0;return g.forEach(T=>{C.forEach(H=>{T===H&&S++})}),S>=g.length}),c},z=(c,l,g)=>(g=g.filter(p=>new Date(p.created_at).getTime()>c.getTime()&&new Date(p.created_at).getTime()<l.getTime()),g),pe=(c,l)=>(c!="All"&&(l=l.filter(g=>c.includes(g.type.toString()))),console.log(l),l),ie=c=>{const l=new Date(c.created_at).getTime(),g=`/map/${c.latitude}/${c.longitude}/${l}`;Y(g)},Z=()=>{var c=`ID,Created At,Type,Location,Text,Radius,latitude,longitude
`;t.forEach(C=>{var ne;var{id:S,created_at:T,type:H,location:J,text:ue,radius:me,latitude:fe,longitude:ge}=C,xe=((ne=a.find(be=>be.id===H))==null?void 0:ne.name)||"Other",ye=`${S},"${T}","${xe}","${J}","${ue}",${me},"${fe},${ge}"
